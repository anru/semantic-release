...
Your configuration for the problematic branches is \`${stringify(branches)}\`.`,
  ...
  EINVALIDNEXTVERSION: ({nextRelease, branch, commits, validBranches}) => ({
    message: `The release ${nextRelease.version} on branch ${branch.name} cannot be published as it is out of range.`,
    details: 'TODO',
  }),
  EINVALIDMAINTENANCEMERGE: ({nextRelease, branch}) => ({message: 'TODO', details: 'TODO'}),
};
